...
# Каталог убирается при любом исходе, а не только при успехе.
#
# /tmp на роутере — это оперативная память, а архив с движком весит полсотни
# мегабайт. Уборка стояла последней строкой, то есть выполнялась лишь когда
# всё прошло; отказ на «не хватает места» оставлял эти пятьдесят мегабайт
# в памяти навсегда. На живом роутере так и вышло: из 484 МБ занято 126 в tmpfs,
# из них 52 — брошенный нами архив, и человек искал, кто съел память.
