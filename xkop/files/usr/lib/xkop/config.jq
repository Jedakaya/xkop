...
def tproxy_inbound:
    {
        ...
        sniffing: {
            ...
            # Имена, у которых адрес назначения подменять нельзя.
            #
            # Список назван в документации движка поимённо, вместе с тем, что
            # ломается без него: push-уведомления iOS перестают приходить,
            # устройства Xiaomi теряют облако, в части игр отваливается
            # голосовой чат. Ломается тихо — телефон просто не звенит, —
            # и связать это с роутером человек не может в принципе.
            #
            # Дополняется своим списком из настроек: у каждого свои устройства,
            # и угадать их за человека нельзя.
            domainsExcluded: (
                ["courier.push.apple.com", "Mijia Cloud", "dlg.io.mi.com"]
                + (settings.sniff_excluded // [])
                | unique
            ),
            routeOnly: true
        }
...
# Резолвер на самом роутере: AdGuard Home, Pi-hole, stubby. Отличается от
# любого другого тем, что зависит от службы, которая может не подняться
# или подняться позже нас. Проверяется по адресу, а не по имени службы:
# ставят их под разными адресами петли, и 127.0.0.10 такой же локальный,
# как 127.0.0.1. Схема резолвера отбрасывается - https://127.0.0.1/dns-query
# ничуть не менее локальный, чем голый адрес.
# Без регулярных выражений: jq в OpenWrt собран без oniguruma, и sub с match
# там не существуют вовсе - программа с ними падает на роутере, но проходит
# на машине разработчика. Схема отрезается разбором по "://", а не шаблоном.
def is_local_resolver($raw):
    ($raw // "") as $s
    # Пустая строка в jq делится в пустой массив, а не в массив из пустой
    # строки, поэтому дальше по цепочке пришёл бы null и split упал бы.
    | if $s == "" then false
      else
        ($s | split("://")) as $parts
        | (if ($parts | length) > 1 then ($parts[1] // "") else ($parts[0] // "") end) as $rest
        | ((($rest | split("/") | .[0]) // "") | split(":") | .[0] // "") as $host
...
def dns_servers:
    ...
    + [ settings.dns_extra[]?
        | ({address: .}
           + (if ($learned | length) > 0 then {unexpectedIPs: $learned} else {} end)) ]
    # Локальный резолвер один - не резолвер.
    #
    # AdGuard Home или Pi-hole поднимается не мгновенно и не всегда раньше нас:
    # порядок запуска служб OpenWrt при одинаковом START не определён вовсе.
    # Пока он поднимается, спрашивать некого: имена не резолвятся, проверка
    # узлов уходит в никуда, и снаружи это выглядит как «все серверы мёртвые».
    # На живом роутере так и было - через тринадцать минут после старта
    # ни одного замера, при полностью исправном туннеле.
    #
    # Поэтому рядом с локальным всегда стоит шифрованный, заданный адресом.
    # Пока локальный не встал, отвечает он; когда встал - отвечает локальный,
    # он ближе, и фильтрация работает. Ставится сам и только когда человек
    # не задал соседа руками: молчаливая зависимость от службы, которая может
    # не подняться, - не то, что стоит оставлять на усмотрение.
    + (if is_local_resolver(settings.dns_server // "")
          and (((settings.dns_extra // []) | length) == 0)
       then [ ({address: "https://8.8.8.8/dns-query"}
               + (if ($learned | length) > 0 then {unexpectedIPs: $learned} else {} end)) ]
       else [] end)
