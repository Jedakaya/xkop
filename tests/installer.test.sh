...
# --- запуск службы нельзя пропускать -----------------------------------------
#
# prepare — единственный путь, которым на роутере появляются правила, резолвер
# и расписание. Обёрнутый в обычный замок, он молча пропускался: обновление
# пакета останавливает службу (правила снимаются), запуск попадает на идущую
# задачу по расписанию, применение пропускается — и весь трафик идёт напрямую.
# Стоило живого роутера через час после того, как замок был добавлен.

cli="$ROOT/xkop/files/usr/bin/xkop"
check "prepare не обёрнут в пропускающий замок" "no"     "$(grep -A 2 'prepare)' "$cli" | grep -q 'lock_run work' && echo yes || echo no)"
