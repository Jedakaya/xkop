...
# Защита от клиентских DoH при выборочном перехвате: порт 853 отдаётся движку,
# иначе правило блокировки в движке его не видит. Без защиты правила нет.
dot=$(nft_ruleset "br-lan" "" 1 0 "198.18.0.0/15 8.8.8.8" 1)
check "порт 853 отдаётся движку при защите" "yes" "$(has "$dot" 'th dport 853 meta mark set')"
check "без защиты порт 853 не трогается" "no" "$(has "$picked" 'th dport 853')"
