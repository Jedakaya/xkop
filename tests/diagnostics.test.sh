...
# Состояние начинается с «правил нет» и продолжается причиной, если она
# известна: на машине без модуля nft_tproxy это будет сказано прямо, и
# требовать точного совпадения строки значит запрещать называть причину.
check "правил нет — так и сказано" "true"     "$(diag_nft_json | jq -r '(.state | startswith("правил нет"))')"
